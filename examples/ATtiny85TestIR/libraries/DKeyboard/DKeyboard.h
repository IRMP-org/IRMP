/*
 * Digispark AVR ATTINY85 Keyboard Library
 *
 * Code taken from DigiKeyboard (based on Obdev's AVRUSB code and modified for Digispark by Digistump)
 *
 * Ref. https://digistump.com/wiki/digispark/tutorials/digikeyboard and
 * https://github.com/digistump/DigistumpArduino/tree/master/digistump-avr/libraries/DigisparkKeyboard
 *
 * Code modified to allow including the DKeyboardDevice class declarations with:
 *    #include "DKeyboard.h"
 * and to separately define the object within the C program:
 *    DKeyboardDevice Db = DKeyboardDevice();
 * The original library (named DigisparkKeyboard) defined the object "DigiKeyboard" within "DKeyboard.h"
 *
 * The license for this code is the same of the forked one.
 */

#ifndef __DKeyboard_h__
#define __DKeyboard_h__
...
class DKeyboardDevice : public Print {
 public:
  DKeyboardDevice () {
    cli();
    usbDeviceDisconnect();
...
// Commented out to allow defining the object within the C program
// DKeyboardDevice DKeyboard = DKeyboardDevice();
...
extern "C"{
  ...
  // usbFunctionSetup code removed to reduce the size
